...
const DesignConfigurator = ({
  ...
  function base64ToBlob(base64: string, mimeType: string) {
    ...
    for (let index = 0; index < byteCharaters.length; index++) {
      byteNumbers[index] = byteCharaters.charCodeAt(index);
    }
                    ...
                    <DropdownMenuContent>
                      {MODELS.options.map((model) => (
                        <DropdownMenuItem
                          key={model.label}
                          className={cn(
                            "flex text-sm gap-1 items-center p-1.5 pr-4 cursor-default hover:bg-zinc-100",
                            {
                              "bg-zinc-100":
                      ...
                      <div className="mt-3 space-y-4">
                        {selectableOptions.map((option) => (
                          <Radio
                            ...
                            className={({ checked }) =>
                              cn(
                                "relative cursor-pointer rounded-lg bg-white px-6 py-4 shadow-sm border-2 border-zinc-200 focus:outline-none focus:ring-0 ring-0 outline-none sm:flex sm:justify-between",
                                {
